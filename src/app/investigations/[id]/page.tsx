...
export default function InvestigationDetailPage({
  ...
  const { id } = use(params);
  const investigation = useSimulationStore((s) =>
    s.investigations.find((c) => c.id === id)
  );
  ...
  const events = data.events
    .filter((e) => e.inventoryItemId === investigation.inventoryItemId)
    .slice(0, 30);
  ...
  return (
    <AppShell
      ...
      description={`Case ${investigation.id} — ${investigation.itemName}`}
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Badge variant={investigation.status === "open" ? "destructive" : "success"}>
            {investigation.status}
          </Badge>
          <Badge variant="outline">{investigation.trigger.replace("_", " ")}</Badge>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Expected Stock", value: investigation.expectedStock },
            { label: "Actual Stock", value: investigation.actualStock },
            { label: "Variance", value: investigation.variance, danger: true },
            { label: "Loss Value", value: formatCurrency(investigation.lossValue), danger: true },
          ].map((m) => (
            <Card key={m.label}>
              ...
              <CardContent>
                <p className={`text-2xl font-semibold tabular-nums ${m.danger ? "text-destructive" : ""}`}>
                  {typeof m.value === "number" ? formatNumber(m.value) : m.value}
                  {typeof m.value === "number" ? ` ${investigation.unit}` : ""}
                </p>
              </CardContent>
            </Card>
          ))}
              ...
              <Card className="border-amber-500/30">
                <CardHeader>
                  <CardTitle className="text-sm font-medium text-amber-500">
                    Suspicious Adjustments ({suspicious.length})
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <EventTimeline events={suspicious} employees={data.employees} inventory={data.inventory} />
                </CardContent>
              </Card>
            )}
          </div>
          ...
          <div className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-3">
                {employees.map((emp) => {
                  if (!emp) return null;
                  const count = events.filter((e) => e.employeeId === emp.id).length;
                  return (
                    <div key={emp.id} className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          {emp.role} · {emp.employeeCode} · {count} events
                        </p>
                      </div>
                    </div>
                  );
